...
export async function addReminderToPhone(payload: PhoneReminderPayload): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    ...
    if (!calendarId) return { ok: false, error: 'Не найден список напоминаний iPhone' };

    const due = new Date(payload.dueAt);
    if (due.getTime() < Date.now()) due.setTime(Date.now() + 5 * 60 * 1000);
    ...
    return { ok: false, error: e instanceof Error ? e.message : 'Не удалось создать напоминание на iPhone' };
  }
}
...
export function promptAddToPhoneAfterReminderSaved(payload: PhoneReminderPayload, onDone?: () => void) {
  Alert.alert('Напоминания iPhone', 'Добавить это напоминание также в приложение «Напоминания» на iPhone?', [
...
export function promptAddReminderForCarePlanTask(
  ...
  Alert.alert('Напоминание к задаче', 'Создать напоминание в приложении PMA и перенести его в «Напоминания» iPhone?', [
    ...
    },
    {
      ...
      onPress: async () => {
        try {
          ...
          const msg = res.ok
            ? 'Напоминание создано в PMA и в «Напоминаниях» iPhone.'
            : `Напоминание в PMA создано. iPhone: ${res.error || 'ошибка'}.`;
...
export function promptTransferCreatedItemsToPhone(
  ...
  Alert.alert(
    options?.title || 'Перенести в iPhone',
    options?.message || `Добавить ${items.length} задач/напоминаний в приложение «Напоминания» на iPhone?`,
    [
...
export { addReminderToPhone as addReminderToIOS, promptAddToPhoneAfterReminderSaved as promptAddToIOSAfterReminderSaved };
