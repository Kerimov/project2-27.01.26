...
exports.stripLeadingSmalltalk = stripLeadingSmalltalk;
exports.classifyAssistantIntent = classifyAssistantIntent;
...
const assistant_medical_intent_1 = require("./assistant-medical-intent");
function norm(input) {
...
function stripLeadingSmalltalk(input) {
    return input
        .replace(/^(?:привет|здравствуй|здорово|добрый\s+(?:день|утро|вечер)|спасибо|благодарю|как\s+дела)[!.?,]?\s*/i, '')
        .trim();
}
function isOnlySmalltalk(input) {
    const t = norm(input);
    if (!t)
        return false;
    return /^(?:привет|здравствуй|здорово|добрый(?:\s+(?:день|утро|вечер))?|спасибо|благодарю|как\s+дела)[!.?\s]*$/i.test(t);
}
function classifyCoreIntent(t) {
    ...
    if ((0, assistant_diary_intent_1.isDiaryWriteIntent)(t)) {
        return { intent: 'diary', confidence: 0.98, reason: 'diary write (priority)' };
    }
    if ((0, assistant_medical_intent_1.isGeneralMedicalQuestion)(t)) {
        return { intent: 'medical_question', confidence: 0.93, reason: 'medical advice question (priority)' };
    }
    if ((0, assistant_diary_intent_1.isDiaryTopicIntent)(t)) {
        return { intent: 'diary', confidence: 0.92, reason: 'diary topic (priority)' };
    }
    if ((0, assistant_diary_intent_1.isAppointmentBookingIntent)(t)) {
        return { intent: 'booking', confidence: 0.95, reason: 'booking request' };
    }
    if (/(?:мои|мой|покажи|какие|когда|ближайш|предстоящ).*(?:запис[ьи]|прием|приёма|визит)|(?:запис[ьи]|прием|приёма|визит).*(?:мои|предстоящ|ближайш|к\s+врач)|запис[ьи]\s*(?:к\s+)?врач/i.test(t)) {
    ...
    if ((0, assistant_analysis_intent_1.isAnalysisDeepDiveRequest)(t)) {
        return { intent: 'medical_question', confidence: 0.95, reason: 'analysis deep dive (RAG+LLM)' };
    }
    if ((0, assistant_analysis_intent_1.isAnalysisListOnlyRequest)(t)) {
...
function classifyAssistantIntent(message) {
    ...
    if (!raw)
        return { intent: 'unknown', confidence: 0, reason: 'empty' };
