...
exports.analyzeMessageAndDetermineFunction = analyzeMessageAndDetermineFunction;
exports.executeLegacyFunction = executeLegacyFunction;
exports.tryLegacyAssistantRouter = tryLegacyAssistantRouter;
const db_1 = require("@/lib/db");
const assistant_diary_intent_1 = require("./assistant-diary-intent");
const assistant_project_actions_1 = require("./assistant-project-actions");
...
async function analyzeMessageAndDetermineFunction(message, userId) {
...
async function extractAppointmentParameters(message, userId) {
    const params = {};
    ...
    try {
        ...
        if (doctorNameMatch) {
            ...
            const foundDoctor = doctors.find(d => d.user.name.toLowerCase().includes(doctorName.toLowerCase()) ||
...
async function extractAnalysisParameters(message) {
    const params = {};
...
async function extractRecommendationParameters(message) {
    const params = {};
...
async function extractDoctorParameters(message) {
    const params = {};
...
async function extractAppointmentQueryParameters(message) {
    const params = {};
...
// Выполнение функций
async function executeLegacyFunction(functionCall, userId) {
    try {
...
async function bookAppointment(params, userId) {
    try {
        console.log('[AI-ASSISTANT] bookAppointment called with params:', params);
        console.log('[AI-ASSISTANT] Prisma availability check:', {
            hasPrisma: !!db_1.prisma,
            hasDoctorProfileModel: !!db_1.prisma?.doctorProfile,
            doctorProfileModelType: typeof db_1.prisma?.doctorProfile
        });
        if (!db_1.prisma || !db_1.prisma.doctorProfile) {
...
async function getAnalysisResults(params, userId) {
    try {
        ...
        const analyses = await db_1.prisma.analysis.findMany({
            where: whereClause,
            ...
            if (analysis.results) {
                try {
                    ...
                    if (results.indicators && results.indicators.length > 0) {
                        message += `   📈 Показатели:\n`;
                        results.indicators.slice(0, 3).forEach((indicator) => {
...
async function getAppointments(params, userId) {
    try {
        ...
        appointments.forEach((appointment, index) => {
            ...
            message += `   📊 Статус: ${(0, assistant_project_actions_1.getStatusLabel)(appointment.status)}\n`;
...
async function tryLegacyAssistantRouter(message, userId) {
